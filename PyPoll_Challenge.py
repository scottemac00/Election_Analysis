...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_votes:
        ...
        vote_percentage = float(votes_by_county) / float(total_votes) * 100 

         # 6d: Print the county results to the terminal.
        county_results = (
          f"{county_name}: {vote_percentage:.1f}% ({votes_by_county:,})\n")
        print(county_results)
         # 6e: Save the county votes to a text file.
        txt_file.write(county_results)
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (votes_by_county > county_turnout):
            county_turnout = votes_by_county
            largest_turnout = county_name
            
    # 7: Print the county with the largest turnout to the terminal.
    largest_county_summary = (
        f"----------------------\n"
        f"Largest Tournout: {largest_turnout}\n"
        f"----------------------\n")
